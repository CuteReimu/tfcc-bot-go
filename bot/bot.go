...
func Init() {
	...
	if deviceJSONContent == nil {
		deviceJSONContent = GenRandomDevice()
	}
...
func InitWithOption(option InitOption) error {
	...
	err := deviceInfo.ReadJson(option.DeviceJSONContent)
	if err != nil {
		return errors.Errorf("failed to apply device.json with err:%s", err)
	}
	Instance.QQClient.UseDevice(deviceInfo)
	return nil
}
...
func InitEmptyClient() {
	...
	deviceJSONContent := utils.ReadFile("./device.json")
	deviceInfo := &client.DeviceInfo{}
	err := deviceInfo.ReadJson(deviceJSONContent)
	if err != nil {
		panic("failed to apply device.json with err:" + err.Error())
	}
	Instance.QQClient.UseDevice(deviceInfo)
}
...
func GenRandomDevice() []byte {
	deviceInfo := client.GenRandomDevice()
	b, _ := utils.FileExist("./device.json")
	if b {
		logger.Warn("device.json exists, will not write device to file")
		return nil
	}
	deviceJSONContent := deviceInfo.ToJson()
	err := os.WriteFile("device.json", deviceJSONContent, os.FileMode(0755))
	if err != nil {
		logger.WithError(err).Errorf("unable to write device.json")
		return nil
	}
	return deviceJSONContent
}
